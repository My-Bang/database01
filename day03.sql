...
-- 1.  사원정보(Employees)테이블에서 사원번호(employee_id), 이름(first_name, last_name) , 급여(salary), 업무(job_id) , 입사일(hire_date), 상사(manager_id)의 사원번호를 출력하시오. 이때 이름은 성과 이름을 연결하여 Name이라는 별칭으로 출력하세요
...
select now() from dual;		-- 현재 날짜와 시간 확인하기
